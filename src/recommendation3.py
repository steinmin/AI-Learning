...
class RecommendAlgr(object):
    ...
    def add_record(self, user, item, tag):
        ...
        self.train_data[user].setdefault(item, [])
        self.train_data[user][item].append(tag)
    ...
    def train_test_split(self, ratio=0.2, random_state=40):
        ...
        for user in self.train_data.keys():
            for item in self.train_data[user].keys():
                if random.random()<ratio: # ratio比例设置为测试集
                    ...
                    self.test_data[user][item] = self.train_data[user][item]
                    self.train_data[user][item] = None
                else:
                    for tag in self.train_data[user][item]:
                        RecommendAlgr.addValueToMat(self.user_tags,  user, tag)
                        RecommendAlgr.addValueToMat(self.tag_items,  tag,  item)
                        RecommendAlgr.addValueToMat(self.user_items, user, item)
                        RecommendAlgr.addValueToMat(self.tag_users,  tag,  user)
        ...
        print("训练集样本数 %d, 测试集样本数 %d" % (len(self.train_data),len(self.test_data)))
